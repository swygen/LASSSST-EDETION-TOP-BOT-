...
# ==========================================
API_TOKEN = '8246035422:AAGJbWJLEJS5-No20RJPG9-KWY0nBxhKMq8'  # ⚠️ টোকেন দিন
ADMIN_ID = 6243881362
...
LANG = {
    "bn": {
        ...
        "sub_p1": "🥉 বেসিক - ১০০৳ দৈনিক 40 টা (৭ দিন)", "sub_p2": "🥈 স্ট্যান্ডার্ড - ২৫০৳ দৈনিক 60 টা (১৫ দিন)", "sub_p3": "🥇 প্রিমিয়াম - ৭০০৳  দৈনিক আনলিমিটেড (৩০ দিন)",
        "pay_ins": "🛒 **পেমেন্ট ইনভয়েস**\n💰 পরিমাণ: {price}৳\n💳 নগদ (পার্সোনাল): `{number}`\n\n⚠️ **নিয়ম:** এই নাম্বারে আপনার নগদ একাউন্ট থেকে সেন্ড মানি করুন এবং নিচে TrxID দিন।",
    ...
    "ar": {
        ...
        "sub_p1": "🥉 أساسي - 100৳ يوميًا 40 (7 أيام)", "sub_p2": "🥈 قياسي - 250৳ يوميًا 60 (15 يوم)", "sub_p3": "🥇 بريميوم - 700৳ يومي غير محدود (30 يوم)",
    ...
    "hi": {
        ...
        "sub_p1": "🥉 बेसिक - 100৳ दैनिक 40 (7 दिन)", "sub_p2": "🥈 स्टैंडर्ड - 250৳ दैनिक 60 (15 दिन)", "sub_p3": "🥇 प्रीमियम - 700৳ दैनिक असीमित  (30 दिन)",
...
# ==========================================
# 🌟 DYNAMIC FEATURES (A-to-Z Translate)
# ==========================================
...
# 2. 🌐 LANGUAGE
@bot.message_handler(func=lambda m: m.text in ["🌐 ভাষা/Lang", "🌐 Language", "🌐 لغة", "🌐 भाषा", "🌐 ভাষা / Language"])
def change_lang(m):
...
def support_handler(m):
    L = LANG[get_user(m.chat.id).get('lang', 'bn')]
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton(L['sup_btn'], url=f"tg://user?id={ADMIN_ID}"))
...
# 7. ⬇️ DOWNLOAD LOGIC (Real-Time Animation)
...
@bot.message_handler(func=lambda m: any(x in m.text.lower() for x in ["tiktok", "facebook", "instagram", "youtube", "reel"]))
...
def process_link(m):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton("🎬 Video", callback_data=f"d_vid_{m.message_id}"),
        types.InlineKeyboardButton("🎵 Audio", callback_data=f"d_aud_{m.message_id}")
    )
    temp_links[str(uid)] = m.text
    bot.send_message(uid, "📥 **Format:**", reply_markup=markup)
...
def final_dl(c):
    ...
    # 🔥 REAL-TIME ANIMATION (As Requested)
    ...
    try:
        opts = {'quiet': True, 'outtmpl': fn+'.mp4', 'format': 'bestvideo+bestaudio/best'}
        ...
        fb.add(types.InlineKeyboardButton("🌟 Review / ফিডব্যাক", url="https://t.me/swygen_it"))
